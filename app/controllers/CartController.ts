...
export function validateCartBeforeSave(cart: ICart){
  try{
    ...
    return {isValid: false, message: 'Erro genérico no objeto do carrinho. Verifique se todos os campos estão preenchidos'};
  }
}
  ...
  async list(req: Request, res: Response){
    ...
    */
    try{
      ...
      return res.status(200).json({status: ResponseStatus.OK, message: 'Carrinhos(s) encontrado(s).', payload: carts} as ResponseDefault);
    }catch(e: any){
    ...
    */
    ...
    try{
      ...
      if(validation.isValid){
        ...
        return res.status(201).json({status: ResponseStatus.OK, message: 'Carrinho criado com sucesso.', payload: cart} as ResponseDefault);
      }else{
    ...
    */
    ...
    try{
      ...
      if(validation.isValid){
        ...
        if(cart)
          return res.status(200).json({status: ResponseStatus.OK, message: 'Carrinho atualizado com sucesso.'} as ResponseDefault);
        else
         return res.status(404).json({status: ResponseStatus.NOT_FOUND, message: 'Carrinho não encontrado.'} as ResponseDefault);
      }else{
        return res.status(422).json({status: ResponseStatus.INVALID_INFO, message: validation.message} as ResponseDefault);
      }
  ...
  async deleteById(req: Request, res: Response){
    ...
    */
    ...
    try{
      ...
      if(cart)
        return res.status(200).json({status: ResponseStatus.OK, message: 'Carrinho deletado com sucesso encontrado.'} as ResponseDefault);
      else  
        return res.status(404).json({status: ResponseStatus.NOT_FOUND, message: 'Carrinho não encontrado.'} as ResponseDefault);
